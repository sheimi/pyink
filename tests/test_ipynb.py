...
def test_cache_isnt_written_if_no_jupyter_deps_single(
    ...
    with open(nb) as src, open(tmp_nb, "w") as dst:
        dst.write(src.read())
    monkeypatch.setattr(
        "pyink.jupyter_dependencies_are_installed", lambda verbose, quiet: False
    )
    result = runner.invoke(
    ...
    jupyter_dependencies_are_installed.cache_clear()
    monkeypatch.setattr(
        "pyink.jupyter_dependencies_are_installed", lambda verbose, quiet: True
    )
    result = runner.invoke(
        main, [str(tmp_path / "notebook.ipynb"), f"--config={EMPTY_CONFIG}"]
    )
...
def test_cache_isnt_written_if_no_jupyter_deps_dir(
    ...
    with open(nb) as src, open(tmp_nb, "w") as dst:
        dst.write(src.read())
    monkeypatch.setattr(
        "pyink.files.jupyter_dependencies_are_installed", lambda verbose, quiet: False
    )
    result = runner.invoke(main, [str(tmp_path), f"--config={EMPTY_CONFIG}"])
    assert "No Python files are present to be formatted. Nothing to do" in result.output
    jupyter_dependencies_are_installed.cache_clear()
    monkeypatch.setattr(
        "pyink.files.jupyter_dependencies_are_installed", lambda verbose, quiet: True
    )
    result = runner.invoke(main, [str(tmp_path), f"--config={EMPTY_CONFIG}"])
    assert "reformatted" in result.output
